  ...
  <div class="panel">
    ...
    <ul class="rules">
      <li v-for="(r, idx) in localRules" :key="idx">
        ...
        <button class="ghost" @click="move(idx, 1)" :disabled="idx===localRules.length-1">↓</button>
    ...
    <div class="actions">
      ...
      <button class="primary" @click="applySort" :disabled="localRules.length===0">Apply Sort</button>
    </div>
  </div>
</template>
...
const localRules = ref<SortRule[]>(props.rules?.map(({ field, direction }, i) => ({ field, direction, precedence: i + 1 })) ?? [])
watch(
  () => props.rules,
  val => {
    localRules.value = (val ?? []).map(({ field, direction }, i) => ({ field, direction, precedence: i + 1 }))
  }
)

function emitRules() {
  const out = localRules.value.map((r, i) => ({ field: r.field, direction: r.direction, precedence: i + 1 }))
...
function add() {
  ...
  localRules.value = [...localRules.value, { field: newField.value, direction: newDirection.value, precedence: localRules.value.length + 1 }]
  emitRules()
  newField.value = ''
...
<style scoped>
.panel { border: 1px solid #e5e7eb; padding: 0.75rem; border-radius: 6px; max-width: 760px; }
header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; }
.rules { list-style: none; padding: 0; margin: 0 0 0.5rem; display: grid; gap: 0.35rem; }
.rules li { display: grid; grid-template-columns: 1fr 1fr auto auto auto auto; gap: 0.35rem; align-items: center; }
.actions { display: flex; gap: 0.5rem; align-items: center; }
.ghost { background: transparent; border: 1px solid #e5e7eb; padding: 0.25rem 0.5rem; border-radius: 4px; }
